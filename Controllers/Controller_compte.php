...
class Controller_compte extends Controller
{
    ...
    public function action_succes_connexion()
    {
        if (isset($_POST['mail']) && isset($_POST['password']) && $_POST['terms'] === "on") {
            $m = Model::getModel();
            $m->connectToAccount($_POST['mail'], $_POST['password']);
            $userId = $_SESSION['user_id'] ?? null;
            if ($userId) {
                $data = ["success" => "Connexion réussie.", "username" => $m->getUsername($userId)];
            } else {
                $data = ["error" => "Échec de la connexion. Veuillez vérifier vos identifiants."];
            }
        } else {
